...
export interface ServerOptions {
  /**
   * `public` leaves the four account tools unregistered, so a deployment that
   * holds no session cookie also advertises no tool that wants one.
   */
  mode?: Mode;
}

export function createServer(client: WillhabenClient, options: ServerOptions = {}): McpServer {
  const mode = options.mode ?? "full";
  const server = new McpServer(
    { name: "willmehr", version: "0.1.0" },
  ...
  if (mode === "full") registerAccountTools(server, client);
  return server;
}

function instructionsFor(mode: Mode): string {
  const base =
    "Search willhaben.at, Austria's largest classifieds marketplace, for second-hand listings. " +
    "Use willhaben_search for ordinary lookups and willhaben_find_deals when the goal is spotting " +
    "underpriced items. Categories, brands and sizes are opaque numeric ids: resolve them with " +
    "willhaben_discover_filters before filtering on them. Prices are in EUR.";
  const account =
    mode === "full"
      ? "The willhaben_my_* tools read the logged-in account and need WILLHABEN_COOKIE. willhaben " +
        "sessions expire within roughly half an hour, so a 401 there usually means the cookie is " +
        "stale rather than wrong."
      : "This server runs in public mode: it is anonymous, holds no session, and has no account " +
        "tools. Profile, watchlist and conversations are not reachable from here — that needs a " +
        "local install of willmehr with the user's own session cookie.";
  return `${base}\n\n${account}`;
}
